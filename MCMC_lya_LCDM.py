...
other_param_min = np.array([7., 0.5, -2.6, 0.])
other_param_max = np.array([15., 1.5, -2.0, 1.])
...
#### HIGH REDSHIFT
pkl = open(gr_folder+'full_matrix_interpolated_ASTRO.pkl', 'r')
...
#### LOW REDSHIFT
pkl = open(gr_folder+'full_matrix_interpolated_ASTRO_LR.pkl', 'r')
...
for param_index in range(astroparams_number_KRIG):
    X[:,param_index] = np.genfromtxt(gr_folder+'kriging_GRID_2R_astro_18p_HR_noPRACE.dat', usecols=[param_index], skip_header=1)
print X.shape

#### LOW REDSHIFT
X_LR = np.zeros(( grid_lenght_ASTRO_LR, astroparams_number_KRIG ))
for param_index in range(astroparams_number_KRIG):
    X_LR[:,param_index] = np.genfromtxt(gr_folder+'kriging_GRID_2R_astro_LR_noPRACE.dat', usecols=[param_index], skip_header=1)
print X_LR.shape
...
fn = folder+NOMEFILE+"_"+cosmo_prior+"prior_"+DATASET+".dat"
